...
| `coverctl check` | Run coverage, aggregate domains, enforce policy | `-o json` emits machine-readable results; exit code `1` signals policy violations. Use `--show-delta` to display coverage changes. Supports `--fail-under N`, `--ratchet`, and reuses existing `--profile` outputs unless you pass `--force-run`. |
